...
class ProductView(APIView):
    ...
    def post(self, request):
        ...
        try:
            # Pass data to dict and add the new fields to be serialized
            ...
            prodcut = ProductModel.objects.create(title=request.POST.get('title'), author=request.POST.get('author'),
                                        description=request.POST.get('description'),
                                        price=request.POST.get('price'),
                                        seller=seller, category=category)

            product = ProductModel.objects.get(id=prodcut.id)
            return Response(prodcut.id, status=status.HTTP_200_OK if product else status.HTTP_400_BAD_REQUEST)
        except:
